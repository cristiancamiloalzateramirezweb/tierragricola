...
public class municipios extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            List<DTOMunicipio> municipios = daoMunicipio.obtenerMunicipiosDepartamento(id_departamento);
            String lista_municipios = "";            
            for (int i = 0; i < municipios.size(); i++) {
                lista_municipios += "<option value='" + municipios.get(i).getId_municipio() + "'>" + municipios.get(i).getMunicipio() + "</option>";
            }
            out.println(lista_municipios);
        }
    }
